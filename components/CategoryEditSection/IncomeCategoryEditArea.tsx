...
import { IncomeCategoriesDataType } from '@/types/incomeType';

export default function IncomeCategoryEditArea() {
  const { sortedIncomeCategories, setIncomeCategory } = useIncomeCategory();
  const [tmpCategories, setTmpCategories] = useState<string[]>([]);
  const [newCategory, setNewCategory] = useState('');

  useEffect(() => {
    setTmpCategories(sortedIncomeCategories);
  ...
  const handleOnDragEnd = (result: DropResult) => {
    ...
    const updated: IncomeCategoriesDataType = items.map((item, idx) => (
      {
        category: item,
        orderNo: idx + 1
      }
